  ...
  return (
    <div>
      <button
        ...
        className="w-full flex items-center gap-1.5 px-2 py-1 text-xs text-stone-400 hover:text-white hover:bg-stone-800 rounded"
      >
        {hasChildren ? (
          isOpen ? <ChevronDown className="w-3 h-3 flex-shrink-0" /> : <ChevronRight className="w-3 h-3 flex-shrink-0" />
        ) : <span className="w-3" />}
        {isOpen ? <FolderOpen className="w-3.5 h-3.5 text-blue-400 flex-shrink-0" /> : <Folder className="w-3.5 h-3.5 text-stone-500 flex-shrink-0" />}
      ...
      {adding && (
        <div className="px-2 pb-1">
          <input
            ...
            className="w-full px-2 py-1 text-xs bg-stone-800 rounded text-white placeholder-slate-500 outline-none focus:ring-1 focus:ring-blue-500"
          />
        </div>
      )}
